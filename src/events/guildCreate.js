...
module.exports = async (client, guild) => {
  ...
  client.logger.info(`Unicron has joined ${guild.name}`);
  ...
  // Self-assign color
  try {
    const unicronColor = guild.roles.cache.find(r => r.name === '');
    if (unicronColor) await guild.me.roles.add(unicronColor);
  } catch (err) {
    client.logger.error(err.message);
  }
